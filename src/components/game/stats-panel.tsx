    ...
    <Card className="flex-shrink-0 bg-card/70 backdrop-blur-sm">
      <CardContent className="p-3 text-center space-y-2">
        <div className="border-b border-dashed pb-2 mb-2">
          <span className="font-bold">{state.playerName}</span>, Rank: <span className="text-primary font-black text-lg">{levels[levelIndex]}</span>
        </div>
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          >
            🏆 {myRank ? `Global Rank: ${formatNum(myRank)}` : 'Connecting...'}
          </Badge>
        ...
        <div className="text-xs font-bold opacity-70 -mt-2">
          Manually Roasted: {formatNum(manualTotal)}
        </div>

        <div className="flex justify-center gap-4 pt-2 border-t border-dashed">
          <StatRow label="👆 Per Click" value={formatNum(currentClick)} badgeClass="bg-[#ffe0b2] text-[#e65100] border-[#ffb74d] hover:bg-[#ffe0b2]/80" />
          <StatRow label="⚙️ Per Second" value={formatNum(currentBps)} badgeClass="bg-[#e0f2f1] text-[#00695c] border-[#4db6ac] hover:bg-[#e0f2f1]/80" />
        </div>
        
        <div className={`text-sm font-bold h-5 transition-all duration-500 ${goldenBean.active ? 'gold-alert' : 'opacity-80'}`}>
          {goldenBean.active ? "✨ Golden Bean Appeared! ✨" : message}
        </div>
      </CardContent>
    </Card>
  );
